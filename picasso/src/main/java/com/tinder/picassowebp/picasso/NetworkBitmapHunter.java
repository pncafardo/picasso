...
import com.tinder.picassowebp.picasso.Downloader.Response;
import static com.tinder.picassowebp.picasso.Picasso.LoadedFrom.DISK;
import static com.tinder.picassowebp.picasso.Picasso.LoadedFrom.NETWORK;
...
class NetworkBitmapHunter extends BitmapHunter {
  ...
  private Bitmap decodeStream(InputStream stream, Request data) throws IOException {
    ...
    if (data.uri.toString().endsWith("webp") || isWebPFile) {
      byte[] bytes = Utils.toByteArray(stream);
      if (calculateSize) {
//        BitmapFactory.decodeByteArray(bytes, 0, bytes.length, options);
        calculateInSampleSize(data.targetWidth, data.targetHeight, options);
      }
      return WebPFactory.nativeDecodeByteArray(bytes, options);
